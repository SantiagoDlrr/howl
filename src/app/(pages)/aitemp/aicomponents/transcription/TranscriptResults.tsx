...
//       <div className="flex justify-between items-start">
//         {/* Example: Instead of a "title" from sections, you could show data.id or a custom label */}
//         <div>
//           <h1 className="text-2xl font-semibold">Call Analysis (ID: {data.id})</h1>
//           {/* You can display any other relevant meta info here if desired */}
//         </div>
...
//               </div>
//               <div className="p-4 bg-white">
//                 <p className="text-gray-700">{data.report.summary}</p>
//               </div>
//             </div>
//           )}
...
//               </div>
//               <div className="p-4 bg-white">
//                 <p className="text-gray-700">{data.report.feedback}</p>
//               </div>
//             </div>
//           )}

//           {/* Key Topics */}
//           {data.report?.keyTopics && data.report.keyTopics.length > 0 && (
//             <div className="border border-gray-100 rounded-lg overflow-hidden">
//               <div className="flex items-center bg-white p-4">
//                 <div className="w-8 h-8 flex items-center justify-center bg-purple-200 text-purple-600 rounded-md mr-3">
//                   <span className="text-xl">★</span>
...
//                 {data.report.keyTopics.map((topic, i) => (
...
//           {/* Emotions */}
//           {data.report?.emotions && data.report.emotions.length > 0 && (
//             <div className="border border-gray-100 rounded-lg overflow-hidden">
//               <div className="flex items-center bg-white p-4">
//                 <div className="w-8 h-8 flex items-center justify-center bg-purple-200 text-purple-600 rounded-md mr-3">
//                   <span className="text-xl">★</span>
...
//               </div>
//               <div className="p-4 bg-white space-y-2">
//                 {data.report.emotions.map((emotionText, i) => (
...
//                 </div>
//                 <div>
//                   <h2 className="text-lg font-semibold">Next Steps / Resolution</h2>
//                   <div className="text-xs text-gray-500">AI generated</div>
//                 </div>
//               </div>
//               <div className="p-4 bg-white">
//                 <p className="text-gray-700">{data.report.output}</p>
//               </div>
//             </div>
//           )}
...
//                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
//                   {data.aspects.map((aspect, idx) => {
//                     let sentimentColor = 'bg-gray-100 text-gray-800 border-gray-200';
//                     if (aspect.sentiment === 'Positive')
//                       sentimentColor = 'bg-green-100 text-green-800 border-green-200';
//                     else if (aspect.sentiment === 'Negative')
//                       sentimentColor = 'bg-red-100 text-red-800 border-red-200';
...
import { TranscriptData } from '../../types/types'; // Make sure this path is correct
...
const TranscriptResults: React.FC<TranscriptResultsProps> = ({ data }) => {
  ...
  // Utility to get emotion color - MODIFIED FOR SAFETY
  const getEmotionColor = (emotion: string | undefined | null) => {
    // Use `|| ''` to ensure we always call toLowerCase() on a string.
    // If emotion is undefined, null, or '', it becomes '' before toLowerCase().
    ...
    switch (lowerCaseEmotion) {
      ...
      case 'mixed':
        return 'bg-yellow-100 text-yellow-800';
      // The default case will now handle empty strings resulting from undefined/null input
      default:
        return 'bg-gray-100 text-gray-800';
    }
  };

  // Render null or a loading state if data is not yet available (optional but recommended)
  if (!data) {
     // You could return a loading spinner here, e.g., return <div>Loading...</div>;
     return null; // Or simply render nothing until data is ready
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-2xl font-semibold">Call Analysis (ID: {data.id ?? 'N/A'})</h1>
          {/* You can display any other relevant meta info here if desired */}
        </div>

        {/* Display emotion only if it exists */}
        {data.oci_emotion && (
          <div className={`px-3 py-1 rounded-md ${getEmotionColor(data.oci_emotion)} text-sm font-medium`}>
            {data.oci_emotion}
          </div>
        )}
        {/* If emotion might be missing, you might want a placeholder or nothing */}
         {!data.oci_emotion && (
           <div className={`px-3 py-1 rounded-md ${getEmotionColor(data.oci_emotion)} text-sm font-medium`}>
             Unknown
           </div>
      ...
      {activeTab === 'summary' ? (
        // ---------- SUMMARY TAB ----------
        <div className="space-y-4">
          {/* Summary - Added optional chaining ?. for safety */}
          {data.report?.summary && (
            <div className="border border-gray-100 rounded-lg overflow-hidden">
              <div className="flex items-center bg-white p-4">
                <div className="w-8 h-8 flex items-center justify-center bg-purple-200 text-purple-600 rounded-md mr-3">
          ...
          {/* Feedback - Added optional chaining ?. */}
          {data.report?.feedback && (
            <div className="border border-gray-100 rounded-lg overflow-hidden">
              <div className="flex items-center bg-white p-4">
                <div className="w-8 h-8 flex items-center justify-center bg-purple-200 text-purple-600 rounded-md mr-3">
          ...
          {/* Overall Sentiment or Output - Added optional chaining ?. */}
          {data.report?.output && (
            <div className="border border-gray-100 rounded-lg overflow-hidden">
              <div className="flex items-center bg-white p-4">
                <div className="w-8 h-8 flex items-center justify-center bg-purple-200 text-purple-600 rounded-md mr-3">
          ...
          {/* Key Phrases (aspects) - Already safe */}
          {data.oci_aspects && data.oci_aspects.length > 0 && (
            <div className="border border-gray-100 rounded-lg overflow-hidden">
              <div className="flex items-center bg-white p-4">
                <div className="w-8 h-8 flex items-center justify-center bg-purple-200 text-purple-600 rounded-md mr-3">
                  <span className="text-xl">★</span>
              ...
              <div className="p-4 bg-white">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {data.oci_aspects.map((aspect, idx) => {
                    // Add nullish coalescing for safety on aspect properties if needed
                    const sentiment = aspect.sentiment ?? 'Unknown';
                    const text = aspect.text ?? 'Missing Text';
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-xs mt-1 opacity-75">{sentiment}</div>
                      </div>
                    );
                  })}
                </div>
        ...
        <div className="border border-gray-100 rounded-lg overflow-hidden">
          <div className="flex items-center justify-between bg-white p-4">
            ...
            {/* Ensure transcript exists before showing copy button */}
            {data.transcript_text && (
              <button
                onClick={() => {
                  if (navigator.clipboard) {
                    navigator.clipboard.writeText(data.transcript_text || '') // Use empty string fallback
                      .then(() => alert('Transcript copied to clipboard!'))
                      .catch(err => console.error('Failed to copy transcript: ', err));
                  } else {
                    alert('Clipboard API not available.'); // Fallback for older browsers/http
                  }
                }}
                className="text-sm text-purple-600 hover:text-purple-800 flex items-center"
              >
          ...
          <div className="p-4 bg-white">
            <div className="bg-gray-50 p-4 rounded-lg max-h-96 overflow-y-auto">
              <p className="text-gray-700 whitespace-pre-wrap leading-relaxed font-mono text-sm">
                {/* Use nullish coalescing for transcript display */}
                {data.transcript_text ?? 'Transcript not available.'}
              </p>
            </div>
          </div>
        </div>
