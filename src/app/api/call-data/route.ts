...
//   const [h, m, s] = durationStr.split(':').map(n => parseInt(n, 10) || 0);
...
//     const callContext            = context ?? `Uploaded call: ${name ?? 'Untitled'}`;
//     const callSatisfaction       = rating;
//     const callDurationInSeconds  = durationToSeconds(duration);
//     const callSummary            = summary;
//     const callDate               = formatDateForDB(date);
...
//     const callRiskWords          = riskWords ? `{${riskWords.map(w => `"${w}"`).join(',')}}` : null; // text[]
...
//     // Depending on how your procedure returns the ID, it may be
//     // under call_id or insert_call_with_emotions
//     const row                    = rows[0] ?? {};
//     const callId: number | null  =
//       (row as any).call_id ?? (row as any).insert_call_with_emotions ?? null;

//     if (!callId) {
//       console.error('❌  Failed to retrieve call ID from procedure result.');
//       return NextResponse.json(
...
export async function POST(request: Request) {
  try {
    ...
    const callDate              = formatDateForDB(date);
    const callTranscript        = full_transcript_text;
    const callMainIdeas         = keyTopics?.length ? keyTopics : null;           // text[]

    /* ---------- Risk words ----------
     * Toggle between HARD‑CODED and DYNAMIC handling by (un)commenting the next two lines.
     */
    const callRiskWords: string[] | null = ['compliance', 'termination', 'lawsuit']; // ←‑‑ HARD‑CODE (testing)
    // const callRiskWords: string[] | null = riskWords?.length ? riskWords : null;    // ←‑‑ DYNAMIC (normal)

    const callEmotionNames      = emotions?.length ? emotions : null;             // text[]
    const callType              = 'Uploaded Audio';
    const callFeedback          = feedback;
    const callSentimentAnalysis = sentiment;
    const callOutput            = output;
    ...
    const rows = await query<{ insert_call_with_emotions: number }>(
      ...
      [
        ...
        callFeedback,
        callSentimentAnalysis,
        callRiskWords,     // text[] or null
        callOutput,
        callEmotionNames   // text[] or null
      ]
    );
    ...
    const row    = rows[0] ?? {};
    const callId = (row as any).call_id ?? (row as any).insert_call_with_emotions ?? null;
